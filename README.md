...
- 已压缩过的文件（文件名含 `_compressed` 或 `_converted`）会自动跳过

## 安全提示

- 当 `keep_original` 设为 `false` 时，程序会显示警告
- **原图会被直接覆盖，无法恢复，请谨慎操作**
- 建议先用 `true` 测试效果，确认无误后再改为 `false`
...
**Q: 为什么有些文件被跳过了？**
A: 文件名含 `_compressed` 或 `_converted` 的文件会被自动跳过，避免重复压缩
